...
export function validateGetStaticPathsResult(result: GetStaticPathsResult, route: RouteData) {
	...
	result.forEach((pathObject) => {
		...
		// TODO: do I really need to check it here? Won't `validateGetStaticPathsParameter` be called somewhere else?
		for (const [key, val] of Object.entries(pathObject.params)) {
			validateGetStaticPathsParameter([key, val], route.component);
		}
	});
}
